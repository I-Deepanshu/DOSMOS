...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      try {
        ...
        const other = chat.participants?.find((p: any) => p.role === "admin");
        setAdminUser(other || null);

        const { data: msgData } = await api.get(`/chats/${chat._id}/messages?page=1&limit=30`);
        ...
        socket.on("new_message", (msg: Message) => {
          ...
          if (senderId !== user?.id) {
            ...
            if (document.visibilityState === "hidden" && "Notification" in window && Notification.permission === "granted") {
              const senderName = typeof msg.sender_id === "object" ? msg.sender_id.name : "Sentinel";
            ...
            socket.emit("message_delivered", { chatId: chat._id, messageId: msg._id });
          }
          setMessages((prev) => {
            if (msg.tempId) {
  ...
  useEffect(() => {
    if (firstUnreadId && messageRefs.current[firstUnreadId]) {
      ...
      setFirstUnreadId(null); // Clear after scrolling so new messages don't force snap back
  ...
  const scrollToMessage = (msgId: string) => {
    ...
    el.classList.add("ring-2", "ring-[var(--user-accent)]", "ring-offset-1");
    setTimeout(() => el.classList.remove("ring-2", "ring-[var(--user-accent)]", "ring-offset-1"), 1500);
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-[100dvh] relative bg-[var(--bg-base)] overflow-hidden">
        {/* Skeleton Header */}
        <div className="flex-none px-6 py-5 border-b border-[var(--border-soft)] bg-[var(--bg-glass)] flex items-center justify-between">
  ...
  return (
    <div className="flex flex-col h-[100dvh] relative bg-[var(--bg-base)] text-[var(--text-primary)] font-['Space_Grotesk']">
      {/* Header */}
      <header 
        className="flex-none px-6 py-5 border-b border-[var(--border-soft)] bg-[var(--bg-glass)] backdrop-blur-md flex items-center justify-between z-10 cursor-pointer hover:bg-[rgba(255,255,255,0.02)] transition-colors"
        onClick={() => setShowInfo(true)}
      >
        <div className="flex items-center gap-4">
          {user?.planet && (
            <div className="w-10 h-10 rounded-full shadow-[0_0_14px_rgba(255,255,255,0.08)]"
                 style={{ background: user.planet.color }} />
          )}
          <div className="flex flex-col">
            <h1 className="text-[13px] uppercase tracking-widest text-[var(--text-secondary)] font-medium">Sentinel Chamber</h1>
      ...
      >
        ...
        {messages.length === 0 && !loading && (
          <div className="m-auto flex flex-col items-center justify-center">
            ...
            <p className="text-[16px] text-[var(--text-primary)] font-medium mb-1">Secure channel ready</p>
            <p className="text-[13px] text-[var(--text-secondary)] opacity-70">Awaiting commander transmission...</p>
          </div>
        )}
              ...
              <Fragment key={msg._id || msg.tempId || i}>
                ...
                {msg.is_system ? (
                  ...
                  <div className="flex flex-col">
                    ...
                    />
                    {isMine && msg._id === lastSeenOutgoingId && msg.seen_at && (
      ...
      <div className="flex-none border-t border-[var(--border-soft)] bg-[var(--bg-glass)] backdrop-blur-xl relative z-20">
        ...
        <AnimatePresence>
          {replyTarget && (
            ...
            >
              <div className="flex items-center justify-between bg-[var(--bg-surface)] border border-[var(--border-soft)] border-l-[3px] border-l-[var(--user-accent)] rounded-[10px] px-4 py-2.5">
                <div className="flex flex-col min-w-0">
                  <span className="text-[10px] uppercase tracking-widest text-[var(--user-accent)] font-semibold mb-0.5">
                ...
                <button
                  ...
                  className="ml-3 shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[rgba(255,255,255,0.05)] transition-colors"
                >
            ...
            {previewFile ? (
              ...
              <>
                ...
                <input
                  ...
                  placeholder={replyTarget ? "Type your reply..." : "Message Sentinel..."}
                  value={input}
...
// ─────────────────────────────────────────────────────────────────────────────
// Swipeable Bubble Component
// ─────────────────────────────────────────────────────────────────────────────
function SwipeableBubble({ msg, isMine, isSameSender, prevMsg, onReply, onScrollToReply, msgRef }: {
  msg: Message;
  isMine: boolean;
  ...
  const replyIconOpacity = useTransform(x, isMine ? [-60, -30, 0] : [0, 30, 60], isMine ? [1, 0.5, 0] : [0, 0.5, 1]);
  const replyIconScale   = useTransform(x, isMine ? [-60, -30, 0] : [0, 30, 60], isMine ? [1, 0.7, 0.4] : [0.4, 0.7, 1]);
  const [showCopyMenu, setShowCopyMenu] = useState(false);
  const THRESHOLD = typeof window !== 'undefined' && window.innerWidth < 640 ? 50 : 40;

  function handleDragEnd() {
    const val = x.get();
    const triggered = isMine ? val < -THRESHOLD : val > THRESHOLD;
    if (triggered) {
  ...
  return (
    <motion.div
      ...
      className={`flex w-full group ${isMine ? "justify-end" : "justify-start"} ${isSameSender && !prevMsg?.is_system ? "mt-1" : "mt-3"} relative`}
    >
      {/* Reply arrow icon — left for isMine swipe, right for theirs */}
      <motion.div
        style={{ opacity: replyIconOpacity, scale: replyIconScale }}
        className={`absolute top-1/2 -translate-y-1/2 ${isMine ? "left-0" : "right-0"} flex items-center justify-center w-9 h-9 rounded-full bg-[var(--bg-surface)] border border-[var(--border-soft)] pointer-events-none z-10`}
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--user-accent)]">
          <polyline points="9 17 4 12 9 7"/><path d="M20 18v-2a4 4 0 0 0-4-4H4"/>
        </svg>
      </motion.div>

      <div className={`relative max-w-[88%] sm:max-w-[68%] flex gap-2 items-center ${isMine ? "flex-row" : "flex-row-reverse"}`}>

        {/* Context Menu (Copy + Reply) */}
        ...
        <motion.div
          drag="x"
          dragConstraints={isMine ? { left: -80, right: 0 } : { left: 0, right: 80 }}
          dragElastic={0.25}
          ...
          className={`px-[18px] py-[13px] rounded-[16px] text-[15.5px] leading-relaxed transition-colors border relative min-w-[80px]
              ${isMine
                ? 'bg-[var(--user-accent)] border-transparent text-[var(--text-primary)] rounded-br-[4px]'
                : 'bg-[var(--bg-surface)] border-[var(--border-soft)] text-[var(--text-primary)] rounded-bl-[4px] shadow-[0_0_10px_var(--sentinel-accent)]'}
        ...
        >
          ...
          {replySnippet && (
            <button
              ...
              className="w-full text-left mb-3 px-3 py-2.5 rounded-[10px] border-l-[3px] border-l-[var(--user-accent)] bg-[rgba(255,255,255,0.04)] hover:bg-[rgba(255,255,255,0.07)] transition-colors"
            >
              <p className="text-[11px] uppercase tracking-wider text-[var(--user-accent)] font-semibold mb-0.5">
